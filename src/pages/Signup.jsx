...
export default function Signup() {
  ...
  const handleSubmit = () => {
    ...
    if (!emailPattern.test(newUserDetails.Email)) {
      errorMessage += 'Please enter a valid email address. ';
    }
    // Validate phone number format (example for US numbers)
    const phonePattern = /^\+?[1-9]\d{1,14}$/; // Allows international format (E.164 standard)
    if (!phonePattern.test(newUserDetails.PhoneNumber)) {
